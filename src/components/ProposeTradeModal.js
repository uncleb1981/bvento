...
export default function ProposeTradeModal({ targetBike, myBikes, submitting, onCancel, onConfirm }) {
  ...
  const [offerMode, setOfferMode] = useState(hasBikes ? 'bike' : 'cash');
  const [myBikeId, setMyBikeId] = useState(myBikes[0]?.id || '');
  const [cashAmount, setCashAmount] = useState(0);
  const [cashDirection, setCashDirection] = useState('even');
  const [cashOnlyAmount, setCashOnlyAmount] = useState(targetBike.estimatedValue);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
      <div className="rounded-t-2xl sm:rounded-2xl p-7 max-w-md w-full max-h-[90vh] overflow-y-auto animate-pop-in" style={{ backgroundColor: 'var(--surface)' }}>
        <h2 className="font-serif text-2xl mb-1" style={{ color: 'var(--ink)' }}>Propose a deal</h2>
        <p className="text-sm mb-5" style={{ color: 'var(--ink-soft)' }}>
          {offerMode === 'cash' ? `Make a cash offer for ${targetBike.title}.` : `Offer one of your bikes for ${targetBike.title}.`}
        </p>

        {hasBikes && (
          <div className="flex gap-1 mb-5 border-b" style={{ borderColor: 'var(--border)' }}>
            <button
              onClick={() => setOfferMode('bike')}
              className="px-3 py-2 text-xs uppercase tracking-[0.1em] font-medium -mb-px"
              style={offerMode === 'bike' ? { color: 'var(--ink)', borderBottom: '2px solid var(--accent)' } : { color: 'var(--ink-soft)', borderBottom: '2px solid transparent' }}
            >
              Offer a bike
            </button>
            <button
              ...
              className="px-3 py-2 text-xs uppercase tracking-[0.1em] font-medium -mb-px"
              style={offerMode === 'cash' ? { color: 'var(--ink)', borderBottom: '2px solid var(--accent)' } : { color: 'var(--ink-soft)', borderBottom: '2px solid transparent' }}
            >
              Cash only
            </button>
          </div>
        ...
        {offerMode === 'cash' && (
          <>
            <div className="rounded-lg overflow-hidden relative h-28 mb-5" style={{ border: '1px solid var(--border)' }}>
              <img src={photoForBike(targetBike)} alt={targetBike.title} className="absolute inset-0 w-full h-full object-cover" />
              <div className="absolute inset-0 flex flex-col justify-end p-3 text-white" style={{ background: 'linear-gradient(to top, rgba(14,16,20,0.85), transparent 70%)' }}>
                <div className="text-sm font-semibold truncate">{targetBike.title}</div>
                <div className="text-xs">Listed around ${targetBike.estimatedValue.toLocaleString()}</div>
              </div>
            </div>

            <div className="mb-6">
              <label className="block text-xs uppercase tracking-[0.1em] font-medium mb-1.5" style={{ color: 'var(--ink-soft)' }}>Your cash offer</label>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setCashOnlyAmount(e.target.value)}
                  className="w-full pl-6 pr-3 py-2.5 text-sm"
                  style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}
                />
              </div>
              <p className="text-xs mt-1.5" style={{ color: 'var(--ink-soft)' }}>
                No bike required — just a straight cash purchase.
              </p>
            </div>
          </>
        )}
