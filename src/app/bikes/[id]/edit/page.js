...
import { getBike, updateBike, getCurrentUser } from '@/lib/store';
...
export default function EditBikePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (!currentUser) {
        router.replace(`/login?next=/bikes/${id}/edit`);
        return;
      }
      const bike = await getBike(id);
      if (cancelled) return;
      if (!bike || bike.ownerId !== currentUser.id) {
        setLoadError('That listing doesn’t exist or isn’t yours to edit.');
        setCheckingAuth(false);
        return;
      }
      setUser(currentUser);
      setExistingPhoto(bike.photo);
      setForm({
        posterName: bike.ownerName || currentUser.name || '',
        title: bike.title,
        type: bike.type,
        condition: bike.condition,
        estimatedValue: String(bike.estimatedValue),
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    setSubmitError('');
    let photo = existingPhoto || TYPE_PHOTO[form.type];
    let photoWarning = '';
    ...
    if (photoFile) {
      try {
        ...
        photoWarning = err.message?.includes('Bucket not found')
          ? 'Photo storage isn’t set up yet — kept your previous photo instead.'
          : `Photo upload failed (${err.message || 'unknown error'}) — kept your previous photo instead.`;
        photo = existingPhoto || TYPE_PHOTO[form.type];
      }
    }
  ...
  const previewPhoto = photoPreview || existingPhoto || TYPE_PHOTO[form.type];
  ...
  if (loadError) {
    return (
      <div className="max-w-lg mx-auto px-4 py-10 text-center">
        <p className="text-sm" style={{ color: '#8A2A1F' }}>{loadError}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-10">
      <h1 className="font-serif text-4xl mb-1" style={{ color: 'var(--ink)' }}>Edit your bike</h1>
      <p className="text-sm mb-6" style={{ color: 'var(--ink-soft)' }}>Update the details on your listing.</p>
